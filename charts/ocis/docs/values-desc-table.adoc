...
| features.externalUserManagement.oidc.gdprExport.basePath
a| [subs=-attributes]
+string+
a| [subs=-attributes]
`"https://keycloak.owncloud.test"`
| Base URI of keycloak.
| features.externalUserManagement.oidc.gdprExport.clientID
a| [subs=-attributes]
+string+
a| [subs=-attributes]
`"gdpr-exporter"`
| Client ID to authenticate against keycloak with.
| features.externalUserManagement.oidc.gdprExport.clientRealm
a| [subs=-attributes]
+string+
a| [subs=-attributes]
`"master"`
| Realm that the client ID is configured in, usually master.
| features.externalUserManagement.oidc.gdprExport.enabled
a| [subs=-attributes]
+bool+
a| [subs=-attributes]
`false`
| Enables the GDPR export feature for the external OIDC provider. Note, this will only work with KeyCloak as OIDC provider.
| features.externalUserManagement.oidc.gdprExport.insecure
a| [subs=-attributes]
+bool+
a| [subs=-attributes]
`false`
...
| features.externalUserManagement.oidc.gdprExport.userRealm
a| [subs=-attributes]
+string+
a| [subs=-attributes]
`"oCIS"`
