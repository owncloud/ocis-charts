...
| Configure OIDC role assignment. If activate oCIS will read the role assigment from the OIDC token, see  https://owncloud.dev/services/proxy/#automatic-role-assignments Note: Enabling `roleAssignment` will disable `adminUUID`.
...
+object+
a| [subs=-attributes]
`{"admin":"ocisAdmin","guest":"ocisGuest","spaceadmin":"ocisSpaceAdmin","user":"ocisUser"}`
